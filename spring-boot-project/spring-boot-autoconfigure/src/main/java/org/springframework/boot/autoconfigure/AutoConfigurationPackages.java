...
public abstract class AutoConfigurationPackages {
	...
	static class Registrar implements ImportBeanDefinitionRegistrar, DeterminableImports {
		...
		public void registerBeanDefinitions(AnnotationMetadata metadata, BeanDefinitionRegistry registry) {
			register(registry, new PackageImports(metadata).getPackageNames().toArray(new String[0])); // 注册bean，getPackageName 返回了当前主程序类的 同级以及子级的包组件，将SpringBootApplication注解标记的包的定义信息注册到容器中。
		}
	...
	private static final class PackageImports {
		...
		PackageImports(AnnotationMetadata metadata) {
			AnnotationAttributes attributes = AnnotationAttributes
					.fromMap(metadata.getAnnotationAttributes(AutoConfigurationPackage.class.getName(), false)); // 获取注解上的值
			List<String> packageNames = new ArrayList<>(Arrays.asList(attributes.getStringArray("basePackages")));
			for (Class<?> basePackageClass : attributes.getClassArray("basePackageClasses")) {
				packageNames.add(basePackageClass.getPackage().getName());
			}
			if (packageNames.isEmpty()) { // 扫描路径是空
				packageNames.add(ClassUtils.getPackageName(metadata.getClassName())); // metadata当前配置类名称,metadata.getClassName()=application 获取类的包路径 启动类所在包路径
			}
