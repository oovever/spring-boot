...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	protected void onRefresh() {
		super.onRefresh();
		try {
			createWebServer(); // 启动tomcat spring调用
		}
		catch (Throwable ex) {
	...
	private void createWebServer() {
		...
		if (webServer == null && servletContext == null) {
			...
			ServletWebServerFactory factory = getWebServerFactory(); // 返回具体实现类对象，tomcat/jetty
			createWebServer.tag("factory", factory.getClass().toString());
			this.webServer = factory.getWebServer(getSelfInitializer()); // 获取webServer tomcat/jetty
			createWebServer.end();
	...
	protected ServletWebServerFactory getWebServerFactory() {
		...
		String[] beanNames = getBeanFactory().getBeanNamesForType(ServletWebServerFactory.class); // 获取ServletWebServerFactory类型的bean
		...
		return getBeanFactory().getBean(beanNames[0], ServletWebServerFactory.class); // 获取实例化后的bean
	}
